...
//if (isset($_GET["submit"]))
	...
	if (!empty($_GET['paperid']))
			{
			$paperid = $_POST['test_id'];
			$sector=$_POST['sector'];
}

$sql = "SELECT * from sample_paper where paperno='{$paperid}' && sector='{$sector}' ORDER BY RAND()";
...
if ($res = mysqli_query($con, $sql)) {
   ...
   ?>
			...
			<input type="radio" name="ans<?php echo $i; ?>" value="a"/>
				...
				<input type="hidden" name="<?php echo $sector;?>" value="<?php echo $sector;?>" >
	<input type="hidden" name="<?php echo $paper1;?>" value="<?php echo $paper1;?>" >
